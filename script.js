(() => {
  ...
  // Fake unavailable dates — seeded deterministic for a believable calendar.
  ...
  (function seedUnavailable() {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      // ~ one week out of every 5 is "booked"
      const block = Math.floor(i / 7) % 5 === 2;
      const weekendCluster = (d.getDay() === 5 || d.getDay() === 6) && i % 11 === 0;
      if (block || weekendCluster) unavailable.add(toKey(d));
    }
  })();
  ...
  function updateSummary() {
    ...
    if (state.checkOut) {
      ...
      submitBtn.textContent = "Demander à réserver";
    }
  }
